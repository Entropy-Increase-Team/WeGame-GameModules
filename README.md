...
它不是独立运行插件，而是挂载在 `WeGame-plugin` 核心层之上的游戏模块，负责提供洛克王国世界相关的账号、档案、家园、战绩、阵容、交换大厅、换蛋广场、查蛋配种与远行商人能力。
...
| `+换蛋广场` | 查看换蛋广场帖子列表 |
| `+换蛋广场 2` | 查看换蛋广场第 2 页 |
| `+发布换蛋帖 雪怪果实\|上岸蛙` | 发布换蛋帖，用「\|」分隔我有和想要 |
| `+我的换蛋帖` | 查看自己发布的换蛋帖 |
| `+关闭换蛋帖 6658f0f3` | 关闭指定换蛋帖 |
| `+订阅换蛋 想要:上岸蛙` | 订阅换蛋广场新帖通知（群管理员） |
| `+取消换蛋订阅` | 取消换蛋订阅（群管理员） |

换蛋订阅支持的筛选键：`想要`、`我有`、`补充`、`学号`、`搜索`。
